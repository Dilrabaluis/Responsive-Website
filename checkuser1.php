	...
	//	var_dump($_POST);
		...
		$resultadmin=mysqli_fetch_array($sqladmin);
	 	if($result){
			...
			echo "<script>alert('登录成功'); window.location='index1.php'</script>";
		}
	 	else if($resultadmin){
		
			$_SESSION['uname']=$resultadmin['adminame'];
			echo "<script>alert('登录成功'); window.location='index2.php'</script>";
		}else{
			echo "<script>alert('登录失败，请重新登录'); window.location='adminlog1.php'</script>";
		}
	 }else if(isset($_POST['submit1'])){
		 echo "<script>window.location='registion1.php';</script>";
